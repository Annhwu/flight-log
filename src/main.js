...
// ─── Steam ─────────────────────────────────────────────────
function updateSteamDisplay() { document.getElementById('steam-display').textContent = minsToHM(steamMinutes); }
function toggleSteamEdit() {
  ...
  document.getElementById('steam-h-inp').value = Math.floor(steamMinutes / 60);
  document.getElementById('steam-m-inp').value = steamMinutes % 60;
}
async function saveSteam() {
  steamMinutes = (parseInt(document.getElementById('steam-h-inp').value) || 0) * 60
    + (parseInt(document.getElementById('steam-m-inp').value) || 0);
  document.getElementById('steam-input-row').style.display = 'none';
  updateSteamDisplay(); updateTotal();
  await saveToFile();
}

// ─── Total ─────────────────────────────────────────────────
function totalSAMin() { return sessions.reduce((a, s) => a + (s.durationMin || 0), 0); }
function updateTotal() { document.getElementById('total-hours').textContent = minsToHM(steamMinutes + totalSAMin()); }
...
function toggleSession() {
  ...
  if (!activeStart) {
    ...
    btn.textContent = '■ VOL OFF'; btn.classList.add('active');
    document.getElementById('si-label').textContent = 'SESSION EN COURS';
    document.getElementById('si-time').textContent = fmtDate(activeStart) + ' — ' + fmtTime(activeStart);
    elapsedInterval = setInterval(updateElapsed, 1000); updateElapsed();
  } else {
    const end = new Date(); clearInterval(elapsedInterval);
    sessions.unshift({ id: Date.now(), startTs: activeStart.getTime(), endTs: end.getTime(), durationMin: (end - activeStart) / 60000 });
    activeStart = null;
    btn.textContent = '▶ VOL ON'; btn.classList.remove('active');
    document.getElementById('si-label').textContent = 'EN ATTENTE';
    document.getElementById('si-time').textContent = '--:--:--';
    document.getElementById('si-elapsed').textContent = '';
    renderSessions(); updateTotal(); saveToFile();
  }
}
function updateElapsed() {
  if (!activeStart) return;
  document.getElementById('si-elapsed').textContent = '⏱ ' + secsToHMS(Math.floor((Date.now() - activeStart.getTime()) / 1000));
}
...
function updateEditResult(id) {
  ...
  document.getElementById('res-dur-' + id).textContent = res ? durLabel(res.dur) : '--';
}
...
function renderSessions() {
  ...
  list.innerHTML = sessions.map((s, i) => {
    const start = new Date(s.startTs), end = new Date(s.endTs);
...
function toggleEdit(id) {
  const row = document.getElementById('edit-' + id), card = document.getElementById('sc-' + id);
